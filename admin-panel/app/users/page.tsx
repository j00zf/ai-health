...
import { Users, Loader2, Info, RefreshCw, Calendar, ArrowUpRight } from "lucide-react";
...
export default function UsersPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#050505] flex text-white font-sans relative overflow-hidden selection:bg-emerald-500/30 selection:text-emerald-400">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-900 pb-5">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-mono tracking-[0.3em] text-zinc-500 uppercase">Core Registries</span>
            </div>
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            <button 
              onClick={fetchUsers}
              disabled={loading}
              className="p-2.5 bg-zinc-950/40 hover:bg-zinc-900 border border-zinc-850 rounded-xl transition-colors disabled:opacity-40"
            ...
            <div className="bg-zinc-950/40 border border-zinc-850 px-4 py-2 rounded-xl font-mono text-xs text-zinc-400 tracking-wider">
              TOTAL RECORD NODES: <span className="text-emerald-400 font-bold ml-1">{loading ? "..." : users.length}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-950/40 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl relative overflow-hidden">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              ...
              <p className="font-mono text-xs text-zinc-500 tracking-widest uppercase animate-pulse">Querying Database Grid...</p>
            </div>
          ) : users.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-2 text-zinc-500">
              <Info size={24} className="text-zinc-600" />
              <p className="font-mono text-xs tracking-wide">No identity data structures returned from remote vector.</p>
            </div>
          ) : (
            /* Outer containment scroll element with styled tracking system custom scrollbars */
              ...
              <table className="w-full text-left border-collapse text-xs whitespace-nowrap">
                <thead>
                  <tr className="border-b border-zinc-900 bg-white/[0.01] text-zinc-400 font-mono tracking-wider uppercase text-[10px]">
                    ...
                    <th className="p-4 font-semibold text-right">Metrics (H / W)</th>
                    ...
                    <th className="p-4 font-semibold text-right">Provision Date</th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex flex-col">
                          <span className="font-semibold text-zinc-100 group-hover:text-emerald-400 transition-colors text-sm">{user.name}</span>
                          <span className="text-[11px] text-zinc-500 font-mono tracking-tight mt-0.5">{user.email}</span>
                          {user.phone && <span className="text-[10px] text-zinc-600 font-mono mt-0.5">{user.phone}</span>}
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        {user.profileCompleted ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-500 bg-amber-500/5 px-2.5 py-1 rounded-lg border border-amber-500/10">
                            <span className="w-1 h-1 rounded-full bg-amber-500" /> Incomplete
                          </span>
                        )}
                      </td>
                      ...
                      {/* Nickname string */}
                      <td className="p-4 font-medium text-zinc-300">{user.profile?.nickname || <span className="text-zinc-600">-</span>}</td>

                      {/* Age */}
                      <td className="p-4 text-center font-mono">{user.profile?.age || <span className="text-zinc-600">-</span>}</td>

                      {/* Gender string formatting badge */}
                      <td className="p-4 font-mono text-[11px] text-zinc-400 capitalize">
                        {user.profile?.gender || <span className="text-zinc-600">-</span>}
                      </td>
                      ...
                      <td className="p-4 text-right font-mono text-zinc-400">
                        {user.profile?.height || user.profile?.weight ? (
                          <div className="flex flex-col text-[11px]">
                            <span>{user.profile.height ? `${user.profile.height} cm` : "-"}</span>
                            <span className="text-zinc-600 text-[10px] mt-0.5">{user.profile.weight ? `${user.profile.weight} kg` : "-"}</span>
                          </div>
                        ) : (
                          <span className="text-zinc-600">-</span>
                        )}
                      ...
                      <td className="p-4 text-center font-mono font-bold text-zinc-200">{user.profile?.bmi || <span className="text-zinc-600">-</span>}</td>

                      {/* Activity string level text block */}
                      <td className="p-4 text-zinc-400 text-[11px]">
                        {user.profile?.activityLevel ? (
                          <span className="max-w-[120px] truncate block" title={user.profile.activityLevel}>
                            {user.profile.activityLevel}
                          </span>
                        ) : (
                          <span className="text-zinc-600">-</span>
                      ...
                      <td className="p-4 text-zinc-400 text-[11px]">
                        {user.profile?.healthGoal ? (
                          <span className="max-w-[140px] truncate block" title={user.profile.healthGoal}>
                            {user.profile.healthGoal}
                          </span>
                        ) : (
                          <span className="text-zinc-600">-</span>
                      ...
                      <td className="p-4 text-right font-mono text-zinc-500 text-[11px]">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <span>{new Date(user.createdAt).toLocaleDateString([], { year: 'numeric', month: 'short', day: '2-digit' })}</span>
                        </div>
                      </td>
      ...
      {/* Global custom styling hook injecting clean dashboard scrollbar overrides */}
      <style jsx global>{`
        @keyframes fadeIn {
